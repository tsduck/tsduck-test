...
$(tspath tsanalyze) $(fpath "$OUTDIR/$SCRIPT.1.ts") --title "$SCRIPT tsanalyze full" $STDOPT \
    |grep -e 0x00D2 -e 0x00DD -e 0x00DE \
    >"$OUTDIR/$SCRIPT.tsanalyze.1.txt" \
    2>"$OUTDIR/$SCRIPT.tsanalyze.1.log"

$(tspath tsanalyze) $(fpath "$OUTDIR/$SCRIPT.2.ts") --title "$SCRIPT tsanalyze full" $STDOPT \
    |grep -e 0x00D2 -e 0x00DD -e 0x00DE \
    >"$OUTDIR/$SCRIPT.tsanalyze.2.txt" \
